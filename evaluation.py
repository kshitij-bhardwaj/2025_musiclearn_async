...
def start_recording(filename, channels, sample_format, fs, chunk):
    import sys, tty, termios

    input("Press Enter when you are ready to record your audio. Press Enter again when finished.")
    print('Recording...')
    ...
    try:
        while True:
            ...
            # Check for a keyboard press (non-blocking)
            if sys.stdin in select.select([sys.stdin], [], [], 0)[0]:
                _ = sys.stdin.read(1) # Clear the keyboard buffer
...
def mistake_detection(notes_duration, teacher_freq, student_freq, cost_matrix, path):
    ...
    """
    # Use np.nanmean to handle NaN values gracefully
    teacher_mean = np.nanmean(teacher_freq)
    student_mean = np.nanmean(student_freq)

    # Check for valid means before normalization
    if np.isnan(teacher_mean) or np.isnan(student_mean):
        print("Warning: Could not calculate mean frequency for normalization. No mistakes will be detected.")
        return [], []

    normalized_teacher_freq = 12.0 * np.log2(teacher_freq / teacher_mean)
    normalized_student_freq = 12.0 * np.log2(student_freq / student_mean)
    ...
    # Check if student_mistakes_duration is not empty
    if not student_mistakes_duration:
...
def main():
    ...
    channels = 1
    fs = 44100
    # seconds = 60 # Set to a long duration to allow for variable recording time
    filename = "output.wav"
    ...
    start_recording(filename, channels, sample_format, fs, chunk)
    ...
    print(f"Original Teacher Audio Length: {librosa.get_duration(y=teacher_audio, sr=sr):.2f} seconds")
    teacher_trimmed, _ = librosa.effects.trim(teacher_audio, top_db=20)
    print(f"Trimmed Teacher Audio Length: {librosa.get_duration(y=teacher_trimmed, sr=sr):.2f} seconds")
    
    teacher_freq = librosa.yin(teacher_trimmed, fmin=75, fmax=300)
    teacher_freq[teacher_freq == 0] = np.nan
    ...
    print(f"Original Student Audio Length: {librosa.get_duration(y=student_audio, sr=sr):.2f} seconds")
    student_trimmed, _ = librosa.effects.trim(student_audio, top_db=20)
    print(f"Trimmed Student Audio Length: {librosa.get_duration(y=student_trimmed, sr=sr):.2f} seconds")
    
    student_freq = librosa.yin(student_trimmed, fmin=75, fmax=300)
    student_freq[student_freq == 0] = np.nan
